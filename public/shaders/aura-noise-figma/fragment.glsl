...
float map(vec3 p){
  ...
  return length(p+vec3(sin((t*.6)*.1)))*log(length(p)+.9)+cos(q.x+sin(q.z+cos(q.y)))*.5-u_distance;
}

void mainImage(out vec4 fragColor,in vec2 fragCoord){
  vec2 a=(fragCoord.xy-u_resolution.xy)/max(u_resolution.y,u_resolution.x)*-3.*u_scale;
  vec3 cl=vec3(0.);
  float d=2.5;
  ...
  for(float i=0.;i<=(1.+20.*u_detail);i++){
    ...
    vec3 midColor=u_midColor*5.;
    vec3 endColor=u_endColor*7.5;
    vec3 l=startColor+midColor*f*1.+endColor*f*2.;
    float audioBoost=u_audioLevels[int(20.*u_detail-i)]/100.;
    float clamped_center_size=u_center_size>=u_bloom?u_bloom-.1:u_center_size;
    cl=cl*l+smoothstep(u_bloom+audioBoost,clamped_center_size,rz)*1.*l;
    d+=min(rz,1.);
  }
  
  vec4 color=vec4(min(vec3(1.,1.,1.),cl),1.);

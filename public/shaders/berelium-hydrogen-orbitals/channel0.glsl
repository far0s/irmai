...
void print_slider( int id, float v, float mini, float maxi )
{
    ...
    bool mouseAndNoSelect = iMouse.z>.5 && roundBox( iMouse.xy-pos-vec2(sl2,6.), vec2(sl2,3.), 5.) < 0. && select.r == 0.; 
    ...
    float mapped;
    if(id == 0) {
        mapped = mini + (maxi - mini) * (value.r + 0.25);
    } else if(id == 1) {
        mapped = floor(MAIN_MIN + (MAIN_MAX - MAIN_MIN) * (uiSlider(0).r + 0.25));
        mapped = (mapped - 1.0) * value.r + 0.5;
    } else {
        mapped = floor(MAIN_MIN + (MAIN_MAX - MAIN_MIN) * (uiSlider(1).r + 0.25));
        mapped = (mapped - 2.0) * value.r + 0.5;
    }
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    _cursor(42,30) _Q _u _a _n _t _u _m _s
    _slider(20,50, 0, 0.74, MAIN_MIN, MAIN_MAX) _n   // 4
    _slider(20,80, 1, 0.5, 0.0, 1.0) _l              // 2
    _slider(20,110, 2, 1.0, 0.0, 1.0) _m             // 2
